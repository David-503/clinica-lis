            ...
            <div class="md-layout-item md-medium-size-100 md-size-100">
                 <md-card>
                    <md-card-header data-background-color="rose">
                        <h4 class="title">Lista de Secretarias</h4>
                        <p class="category">Administra los datos de las secretarias desde acá</p>
                    </md-card-header>
                    <md-card-content>
                        <md-table class="visible" v-model="searched" md-sort="name" md-sort-order="asc">
                            ...
                            <md-table-empty-state
                                md-label="No hay secretarias"
                                :md-description="`No se han encontrado resultados con la búsqueda: '${search}'. Intentalo de nuevo con una diferente manera.`">
                                <md-button class="md-primary md-raised" @click="$router.push('/dashboard/secretaria/crear')">Crear nueva Secretaria</md-button>
                            </md-table-empty-state>
                            <md-table-row slot="md-table-row" slot-scope="{ item }">
                                ...
                                <md-table-cell md-label="Telefono" md-sort-by="phone">{{ getPhone(item.phones) }}</md-table-cell>
                                <md-table-cell class="flex-center" md-label="Acciones" >
                                    <drop-down>
                                        ...
                                        <ul class="dropdown-menu dropdown-menu-right">
                                            <li><a class="md-warning-hover" @click.prevent="editsecretaria(item)"><md-icon>edit</md-icon> Edit</a></li>
                                            <li><a class="md-danger-hover" @click.prevent="deletesecretaria(item)"><md-icon>delete</md-icon> Delete</a></li>
                                        </ul>
                                    </drop-down>
                                </md-table-cell>
                            </md-table-row>
...
<script>
    ...
    import secretaria from "./Secretaria.vue";
    export default {
        components: {
            secretaria
        },
        ...
        methods: {
            // Get Data
            searchOnTable () {
                this.searched = searchByName(this.secretarias, this.search)
            },
            getPhone(phones){
                let strPhones = "";
            ...
            editsecretaria(Doc){
                Bus.$emit('loading');
                this.$router.push(`/dashboard/secretaria/edit/${Doc.dui}`);
            },
            deletesecretaria(Doc){
                Bus.$emit('loading');
                axios.delete(`/dashboard/secretaria/${Doc.dui}`)
                .then(
                    response => {
        ...
        created () {
            Bus.$emit('loading');
            axios.post('/dashboard/secretaria')
            .then(
                response => {
                    Bus.$emit('stopLoading');
                    this.secretarias = response.data;
                    this.searched = this.secretarias;
                }
            )
            .catch(
                error => {
