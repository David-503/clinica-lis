...
<form novalidate @submit.prevent="validatePatient">
    ...
    <md-card>
        ...
        <md-card-content>
            <h3 class="text-rose">Información del Doctor</h3>
            <div class="md-layout">
                ...
                <div class="md-layout-item md-size-100 text-right">
                    ...
                    <md-button v-if="editMode" type="button" @click="$router.push('/dashboard/doctor')" class="md-simple md-rose">Regresar a registros</md-button>
                    <md-button v-if="!editMode" type="submit" class="md-raised md-rose">Ingresar doctor</md-button>
                    <md-button v-if="editMode" type="submit" class="md-raised md-rose">Editar doctor</md-button>
                </div>
            </div>
        </md-card-content>
    </md-card>
...
export default {
    ...
    computed: {
        actionToDo: function() {
            return (this.editMode) ? `Editar doctor con DUI: ${this.patient.dui}` : "Agregar Nuevo Doctor";
        }
    },
    methods: {
        // Get Data
        ...
        savePatient() {
            Bus.$emit('loading');
            axios.post('/dashboard/doctor/crear', this.patient)
            .then(
                response => {
        ...
        editPatient() {
            Bus.$emit('loading');
            axios.put('/dashboard/doctor/actualizar', this.patient)
            .then(
                response => {
                    Bus.$emit('stopLoading');
                    if(response.status == 200){
    ...
    created() {
        ...
        if(this.editMode && this.$route.params.dui){
            axios.post('/dashboard/doctor/infoDoctor', {dui: this.$route.params.dui})
            .then(
                response => {
                    Bus.$emit('stopLoading');
            ...
            this.$router.push('/dashboard/infoDoctor')
        }
    }
}
</script>
