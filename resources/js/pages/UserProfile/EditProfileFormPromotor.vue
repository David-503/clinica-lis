...
<form novalidate @submit.prevent="validatePatient">
    ...
    <md-card>
        ...
        <md-card-content>
            <h3 class="text-rose">Información del Promotor</h3>
            <div class="md-layout">
                ...
                <div class="md-layout-item md-size-100 text-right">
                    ...
                    <md-button v-if="editMode" type="button" @click="$router.push('/dashboard/promotor')" class="md-simple md-rose">Regresar a registros</md-button>
                    <md-button v-if="!editMode" type="submit" class="md-raised md-rose">Ingresar promotor</md-button>
                    <md-button v-if="editMode" type="submit" class="md-raised md-rose">Editar promotor</md-button>
                </div>
            </div>
        </md-card-content>
    </md-card>
...
export default {
    ...
    computed: {
        actionToDo: function() {
            return (this.editMode) ? `Editar promotor con DUI: ${this.patient.dui}` : "Agregar Nuevo promotor";
        }
    },
    methods: {
        // Get Data
        ...
        savePatient() {
            Bus.$emit('loading');
            axios.post('/dashboard/promotor/crear', this.patient)
            .then(
                response => {
        ...
        editPatient() {
            Bus.$emit('loading');
            axios.put('/dashboard/promotor/actualizar', this.patient)
            .then(
                response => {
                    Bus.$emit('stopLoading');
                    if(response.status == 200){
    ...
    created() {
        ...
        if(this.editMode && this.$route.params.dui){
            axios.post('/dashboard/promotor/infoPromotor', {dui: this.$route.params.dui})
            .then(
                response => {
                    Bus.$emit('stopLoading');
            ...
            this.$router.push('/dashboard/infopromotor')
        }
    }
}
</script>
