...
<form novalidate @submit.prevent="validatePatient">
    ...
    <md-card>
        ...
        <md-card-content>
            <h3 class="text-rose">Información del la Secretaria</h3>
            <div class="md-layout">
                ...
                <div class="md-layout-item md-size-100 text-right">
                    ...
                    <md-button v-if="editMode" type="button" @click="$router.push('/dashboard/secretaria')" class="md-simple md-rose">Regresar a registros</md-button>
                    <md-button v-if="!editMode" type="submit" class="md-raised md-rose">Ingresar secretaria</md-button>
                    <md-button v-if="editMode" type="submit" class="md-raised md-rose">Editar secretaria</md-button>
                </div>
            </div>
        </md-card-content>
    </md-card>
...
export default {
    ...
    computed: {
        actionToDo: function() {
            return (this.editMode) ? `Editar doctor con DUI: ${this.patient.dui}` : "Agregar Nueva secretaria";
        }
    },
    methods: {
        // Get Data
        ...
        savePatient() {
            Bus.$emit('loading');
            axios.post('/dashboard/secretaria/crear', this.patient)
            .then(
                response => {
        ...
        editPatient() {
            Bus.$emit('loading');
            axios.put('/dashboard/secretaria/actualizar', this.patient)
            .then(
                response => {
                    Bus.$emit('stopLoading');
                    if(response.status == 200){
        ...
        reset() {
            if(this.editMode) this.$router.push('/dashboard/secretaria/crear');
            this.editMode = false;
    ...
    created() {
        ...
        if(this.editMode && this.$route.params.dui){
            axios.post('/dashboard/secretaria/infoSecretaria', {dui: this.$route.params.dui})
            .then(
                response => {
                    Bus.$emit('stopLoading');
            ...
            this.$router.push('/dashboard/infoSecretaria')
        }
    }
}
</script>
