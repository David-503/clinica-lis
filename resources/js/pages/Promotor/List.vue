            ...
            <div class="md-layout-item md-medium-size-100 md-size-100">
                 <md-card>
                    <md-card-header data-background-color="rose">
                        <h4 class="title">Lista de promotores</h4>
                        <p class="category">Administra los datos de los promotores desde acá</p>
                    </md-card-header>
                    <md-card-content>
                        <md-table class="visible" v-model="searched" md-sort="name" md-sort-order="asc">
                            ...
                            <md-table-empty-state
                                md-label="No hay promotores"
                                :md-description="`No se han encontrado resultados con la búsqueda: '${search}'. Intentalo de nuevo con una diferente manera.`">
                                <md-button class="md-primary md-raised" @click="$router.push('/dashboard/promotor/crear')">Crear nuevo</md-button>
                            </md-table-empty-state>
                                ...
                                <md-table-cell class="flex-center" md-label="Acciones" >
                                    <drop-down>
                                        ...
                                        <ul class="dropdown-menu dropdown-menu-right">
                                            <li><a class="md-warning-hover" @click.prevent="editPromotor(item)"><md-icon>edit</md-icon> Edit</a></li>
                                            <li><a class="md-danger-hover" @click.prevent="deletePromotor(item)"><md-icon>delete</md-icon> Delete</a></li>
                                        </ul>
                                    </drop-down>
                                </md-table-cell>
                            </md-table-row>
...
<script>
    ...
    export default {
        ...
        methods: {
            // Get Data
            searchOnTable () {
                this.searched = searchByName(this.promotores, this.search)
            },
            getPhone(phones){
                let strPhones = "";
            ...
            editPromotor(Doc){
                Bus.$emit('loading');
                this.$router.push(`/dashboard/promotor/edit/${Doc.dui}`);
            },
            deletePromotor(Doc){
                Bus.$emit('loading');
                axios.delete(`/dashboard/promotor/${Doc.dui}`)
                .then(
                    response => {
        ...
        created () {
            Bus.$emit('loading');
            axios.post('/dashboard/promotor')
            .then(
                response => {
                    Bus.$emit('stopLoading');
                    this.promotores = response.data;
                    this.searched = this.promotores;
                }
            )
            .catch(
                error => {
